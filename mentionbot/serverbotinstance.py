...
class ServerBotInstance:
   ...
   async def get_instance(cls, client, server):
      ...
      modules = await self._prepare_core_module_instances()
   ...
   # Obtains a list of wrapped and activated instances of each core module.
   async def _prepare_core_module_instances(self):
      wrapped_modules = []
      for module_class in self._core_modules:
         wrapped_module = await ServerModuleWrapper.get_instance(module_class, self, core=True)
         await wrapped_module.activate()
